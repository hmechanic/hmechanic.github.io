...
const About = () => {
  ...
  return (
    <section id="about" className="relative overflow-hidden bg-dark-bg px-6 py-20 sm:py-28">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute right-0 top-1/3 h-96 w-96 translate-x-1/2 rounded-full bg-neon-cyan/[0.04] blur-3xl"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-0 bottom-0 h-80 w-80 -translate-x-1/3 rounded-full bg-neon-purple/[0.035] blur-3xl"
      />
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        >
          <h2 className="flex items-center text-3xl font-bold sm:text-4xl">
            ...
            <span
              aria-hidden="true"
              className="ml-6 hidden h-1.5 w-1.5 shrink-0 rotate-45 bg-neon-cyan/80 [box-shadow:0_0_10px_rgba(0,243,255,0.7)] sm:block"
            />
            <span
              aria-hidden="true"
              className="ml-3 h-px max-w-xs flex-grow bg-gradient-to-r from-neon-cyan/40 via-white/15 to-transparent"
            />
          </h2>
        </motion.div>
        ...
        >
          ...
          <div className="mt-9 grid items-start gap-12 lg:grid-cols-[minmax(0,1fr)_15rem] lg:gap-16">
            <div className="relative order-2 text-[1.02rem] leading-8 text-slate-300 md:columns-2 md:gap-10 md:[column-rule:1px_solid_rgba(255,255,255,0.08)] lg:order-1">
              ...
              <span
                aria-hidden="true"
                className="absolute -left-5 top-1 hidden h-14 w-px bg-gradient-to-b from-neon-cyan/60 to-transparent lg:block"
              />
            ...
            <aside className="order-1 mx-auto w-full max-w-xs lg:order-2 lg:sticky lg:top-28 lg:mx-0">
              <figure className="group">
                <div className="relative aspect-[4/5] overflow-hidden border border-white/15 bg-gradient-to-br from-white/[0.035] to-white/[0.01] transition-colors duration-500 group-hover:border-white/25">
                  ...
                  <span
                    aria-hidden="true"
                    className="absolute left-0 top-0 z-10 h-8 w-px bg-neon-cyan/70 transition-all duration-500 group-hover:h-10"
                  />
                  <span
                    aria-hidden="true"
                    className="absolute left-0 top-0 z-10 h-px w-8 bg-neon-cyan/70 transition-all duration-500 group-hover:w-10"
                  />
                  <span
                    aria-hidden="true"
                    className="absolute bottom-0 right-0 z-10 h-8 w-px bg-neon-purple/70 transition-all duration-500 group-hover:h-10"
                  />
                  <span
                    aria-hidden="true"
                    className="absolute bottom-0 right-0 z-10 h-px w-8 bg-neon-purple/70 transition-all duration-500 group-hover:w-10"
                  />
                  {/* Subtle diagonal sheen */}
                  <span
                    aria-hidden="true"
                    className="absolute inset-0 bg-gradient-to-tr from-black/15 via-transparent to-white/[0.05]"
                  />
                </div>
              </figure>
              ...
              <dl className="mt-7 border-t border-white/15">
                {t.about.stats.map((stat, index) => (
                  ...
                  >
                    ...
                    <dt
                      className={`text-right text-[0.62rem] font-medium uppercase tracking-[0.15em] lg:mt-1 lg:text-left ${statAccents[index % 3]}`}
                    >
                      {stat.label}
                    </dt>
                  </motion.div>
